...
import { getLTTBSampledTimeseries } from '~/api/flight/getLTTBSampledTimeseries'
...
export const PropertyList = ({ overallData, activeTimeSeries, customPlots }: PropertyListProps) => {
  ...
  const fetchTimeSeriesOnClick = useMutation(getLTTBSampledTimeseries, {
        ...
        const dataForIDB = {
          ...
          id: `${
            overallData.isIndividualFlight
              ...
              : `${overallData.id}-${messageField}-${messageType}`
          }`
            .toLowerCase()
            .replace(',', '')
            .replace('[', '')
  ...
  const fetchClickedProperty = (messageType: string, messageField: string) => {
    ...
    fetchTimeSeriesOnClick.mutate({
      ...
      messageField,
      flightid: overallData.flightid,
      n_datapoints: 100, //TODO: find a more dynamic solution
    })
  }
